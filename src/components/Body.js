...
const Body=()=>{
    ...
    const fetchData=async()=>{
       const data= await fetch("https://www.swiggy.com/dapi/restaurants/list/v5?lat=28.56405&lng=77.28164&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING");
        ...
        setListofRestauarnt(json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants)
        setFilterListofRestauarnt(json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants)
    }
